	...
	<div>
		...
		<div class="d-flex justify-content-center mt-4 mb-4 mx-4 gap-2">
			<div class="btn-group d-flex gap-4">
				<button @click="showRdvBecome" type="button"
					:class="defaultHistory ? 'btn btn-outline-primary w-100 active' : 'btn btn-outline-primary w-100'">
					Upcoming appointments
				</button>
				<button @click="showRdvPassed" type="button"
					:class="!defaultHistory ? 'btn btn-outline-primary w-100 active' : 'btn btn-outline-primary w-100'">
					Past appointments
				</button>
			</div>
		</div>
		<div class="d-flex  flex-column align-items-center justify-content-center" v-if="listHistory.length > 0 && isPageLoaded === true">
			<div class="w-75" v-for="(history, index) in listHistory">
...
const listHistory = computed(() => historyStore.machineHistory.filter((value) =>
